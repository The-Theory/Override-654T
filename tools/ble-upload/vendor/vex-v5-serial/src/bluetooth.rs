...
/// Keeps the CoreBluetooth session alive for the life of the process.
///
/// Dropping the Manager/Adapter tears down btleplug's event loop, which kills any
/// connection still in use ("Event receiver died"). Long uploads outlive discovery,
/// so discovery cannot own them.
static SESSION: std::sync::OnceLock<(Manager, Adapter)> = std::sync::OnceLock::new();
...
pub async fn find_devices(
    ...
    // Scanning while connected costs throughput on an already slow link.
    adapter.stop_scan().await?;

    drop(events);
    let _ = SESSION.set((manager, adapter));
...
impl BluetoothConnection {
    ...
    async fn receive_one_packet(&mut self) -> Result<(), BluetoothError> {
        loop {
            let Some(notification) = self.notification_stream.next().await else {
...
impl Connection for BluetoothConnection {
    ...
    async fn read_user(&mut self, buf: &mut [u8]) -> Result<usize, BluetoothError> {
        let value = self.peripheral.read(&self.user_tx).await?;
