...
/// Bluetooth writes are fire-and-forget, so nothing stops us from queueing data
/// faster than the Brain commits it to flash. It falls behind, then misses the
/// next handshake and drops the link. Sync on an ack every so often to stay in
/// step with it. Both knobs are hardware-dependent -- tune if uploads stall.
fn ble_ack_interval() -> u32 {
    env_knob("V5_BLE_ACK_INTERVAL", 0)
}

fn ble_chunk_delay() -> Duration {
    Duration::from_micros(env_knob("V5_BLE_CHUNK_DELAY_US", 0) as u64)
}

/// Handshakes right after a large Bluetooth transfer need a far longer window:
/// the Brain is still writing flash and will not answer for seconds.
fn ble_handshake() -> (Duration, usize) {
    (
        Duration::from_millis(env_knob("V5_BLE_HANDSHAKE_MS", 4000) as u64),
        env_knob("V5_BLE_HANDSHAKE_RETRIES", 10) as usize,
    )
}
...
impl Command for DownloadFile {
    ...
    async fn execute<C: Connection + ?Sized>(
        ...
        let (init_timeout, init_retries) = if connection.connection_type() == ConnectionType::Bluetooth {
            ble_handshake()
        } else {
            (Duration::from_millis(500), 5)
        };
...
impl Command for UploadFile<'_> {
    ...
    async fn execute<C: Connection + ?Sized>(
        ...
        let mut offset = 0;
        for (chunk_index, chunk) in self.data.chunks(max_chunk_size as _).enumerate() {
            ...
            // On bluetooth we skip the per-chunk reply, but sync periodically so the
            // Brain's flash writer cannot fall arbitrarily far behind us.
            if connection.connection_type() == ConnectionType::Bluetooth {
                let interval = ble_ack_interval();
                if interval > 0 && chunk_index % interval == interval - 1 {
                    let (timeout, retries) = ble_handshake();
                    connection
                        .handshake::<FileDataWriteReplyPacket>(timeout, retries, packet)
                        .await?
                        .payload?;
                } else {
        ...
        let (exit_timeout, exit_retries) = if connection.connection_type() == ConnectionType::Bluetooth {
            ble_handshake()
        } else {
...
description={}",
        ...
        // PROS names the cold library by a hash of the template set, so an identical
        // library already on the Brain can be linked instead of re-uploaded.
        let program_lib_name = std::env::var("V5_LIB_NAME")
            .unwrap_or_else(|_| format!("{base_file_name}_lib.bin"));
        ...
        if let Some(mut library_data) = library_data {
            ...
            connection
                .execute_command(UploadFile {
                    ...
                    vendor: FileVendor::Dev2, // 'pros' vid
                    data: &library_data,
                    target: FileTransferTarget::Qspi,
                    load_address: USER_PROGRAM_LOAD_ADDR, // cold library lives in cold memory (0x3800000)
                    linked_file: None,
        ...
        if let Some(mut program_data) = program_data {
            ...
            connection
                .execute_command(UploadFile {
                    ...
                    data: &program_data,
                    target: FileTransferTarget::Qspi,
                    load_address: if is_monolith { USER_PROGRAM_LOAD_ADDR } else { PROS_HOT_BIN_LOAD_ADDR }, // hot binary lives in hot memory (0x7800000)
                    linked_file,
